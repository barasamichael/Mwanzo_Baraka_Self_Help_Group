...
class Timer:
    ...
    def __init__(self, begin = datetime.utcnow(), end = datetime.utcnow()):
        self.begin = datetime.utcnow() - timedelta(days = 5 * 365, seconds = 0, 
                milliseconds = 0)
        self.end = end
...
class Generator:
    ...
    def generate_loan(self, member_id = 1, type_id = 1):
        ...
        if deposits and viable:
            ...
            Loan = loan(
                ...
                amount = deposits * Loan_Type.multiplier,
                member_id = member_id,
                date_created = fake_time,
                last_updated = fake_time
            )
    ...
    def generate_phone_number(self, member_id):
        ...
        phone_no = phone_number(                                                                          member_id = member_id,
    ...
    def sleeping_thread(self):
        ...
        while True:
            ...
            for i in range(5):
                ...
                loans = db.session.query(func.sum(loan.loan_id)).scalar()
                if loans:
                    for i in range(loans):
                        self.generator.generate_installment(loan_id = randint(1, loans))
